...
def seed_current_stl(directory):
    """An empty order uses the bundled example propeller."""
    directory = safe_path(directory)
    ...
    if stl_directory.is_dir() and any(
        ...
        return
    source = ROOT / "examples/STL/10x7E.stl"
    if not source.is_file():
        raise ValueError(
            f"Place a propeller STL in {stl_directory}; the example input is missing."
        )
