...
from tools.cfmesh_runtime import (
...
def protected_source_paths():
    """Source/configuration folders must never become simulation output."""
    return tuple((ROOT / name).resolve() for name in (
        ".git", ".codex", ".agents", "tools", "tests", "examples", "Parameters",
        "CoreTemplates", "acousticSolver", "Core Template AMI - kEpsilon",
        "Core Template AMI - kOmegaSST", "Core Template DES - kOmegaSST",
    ))
...
def safe_path(path):
    ...
    key = _path_key(path)
    for original in protected_source_paths():
        original_key = _path_key(original)
        if key == original_key or key.startswith(original_key + "/"):
            raise ValueError(
                f"Source directory cannot contain simulation output: {original}"
            )
...
def _preflight(args):
    ...
    if args.mode not in (None, "AMI", "MRF"):
        raise ValueError(
            "Supported modes are AMI and MRF."
        )
    if args.study:
...
def _prepare_geometry(case, source, acoustic_surface, acoustic_diameter, study=None):
    ...
    from tools.cfmesh_refinement import prepare_refinements
...
def run_mesh(container, case, cores, allow_bad, callback=None, live=False):
    ...
    from tools.cfmesh_interface import project_rotating_interface
