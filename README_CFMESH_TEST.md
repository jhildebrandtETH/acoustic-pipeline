...
This standalone experiment is retained on the cfmesh-standalone branch in the local repository. It uses run_cfmesh_test.sh and cfmesh_test/ independently of the main pipeline.
...
The script runs `cartesianMesh` and `improveMeshQuality` through Docker image `opencfd/openfoam-default:2512`, and `createPatch` / `checkMesh` through `microfluidica/openfoam:13`. No host OpenFOAM installation is needed. Start Docker and pull both images first. Run `python3 cfmesh_runtime.py --smoke-test` for a lightweight runtime and bind-mount check. This no-layer experiment does not invoke the host `generateBoundaryLayers` binary; the main pipeline preflight requires it (install with `bash setup_cfmesh.sh`). The default is four shared-memory threads unless OMP_NUM_THREADS is already set:
...
The runner reads only the repository's `cfmesh_test/input/fused.stl`.
...
If you have Linux ParaView installed in WSL, open the mesh directly:

```bash
cd "$CASE"
paraview cfmesh_test.foam
```
...
6. For a geometry overlay, also open
   `cfmesh_test/input/fused.stl` from the repository and use a contrasting colour
